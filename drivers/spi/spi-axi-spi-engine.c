...
static void spi_engine_set_word_count(struct spi_engine *spi_engine,
					       struct spi_transfer *xfer)
{
	...
	if ((xfer->len * 8) < word_length)
		xfer->len = 1;
	else if (xfer->len % word_len_bytes)
		xfer->len = (xfer->len / word_len_bytes) + 1;
	else
		xfer->len = xfer->len / word_len_bytes;
}
	...
	struct spi_engine *spi_engine, unsigned int clk_div, unsigned int delay)
{
	...
	/* spi_clk is in Hz while delay is usec, a devision is required */
	t /= 1000000;
	while (t) {
	...
	struct spi_message *msg, bool dry, struct spi_engine_program *p)
{
	...
	list_for_each_entry(xfer, &msg->transfers, transfer_list) {
		...
		spi_engine_set_word_count(spi_engine, xfer);
...
static void spi_engine_load_buff(struct spi_engine *spi_engine, uint8_t *buf)
{
	...
	if (spi_engine->word_length % 8)
		len = spi_engine->word_length / 8 + 1;
	else
		len = spi_engine->word_length / 8;

	for (i = 0; i < len; i++)
		buf[i] = val >> (8 * i);
}

static void spi_engine_unload_buff(struct spi_engine *spi_engine,
				   const uint8_t *buf)
{
...
static bool spi_engine_write_tx_fifo(struct spi_engine *spi_engine)
{
	...
	while (n && spi_engine->tx_length) {
		...
		for (i = 0; i < m; i++)
			spi_engine_unload_buff(spi_engine, buf);
		spi_engine->tx_buf += m;
...
static bool spi_engine_read_rx_fifo(struct spi_engine *spi_engine)
{
	...
	while (n && spi_engine->rx_length) {
		...
		for (i = 0; i < m; i++)
			spi_engine_load_buff(spi_engine, buf);
		spi_engine->rx_buf += m;
